...
/// Layout multiple cells like a stack, without creating a stack element.
/// Requires only the spacing to insert between blocks, the stack growth
/// direction, its children, as well as relevant layout information.
///
/// In particular, this doesn't require creating a stack element explicitly,
/// which is constrained to Typst values. Therefore, calling this function
/// allows customizing stack layout more deeply, such as for lists, which need a
/// custom layout function for each list item (a stack child). Each child
/// receives relevant layout data.
